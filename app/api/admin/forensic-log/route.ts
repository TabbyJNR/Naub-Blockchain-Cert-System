 ...
 *   ?certificateId=x  — return all entries for a specific certificate (audit trail)
...
export async function GET(request: Request) {
  try {
    ...
    if (certificateId) {
      // Audit trail: all entries for one certificate
      query.certificateId = certificateId;
    } else if (unreadOnly) {
      // Bell badge: count of unacknowledged flagged alerts
      query.flagged = true;
    ...
    const entries = await ForensicLogModel
      .find(query)
      .sort({ timestamp: -1 })
      .limit(limit)
      .lean();

    // For unread-only, just return the count
    if (unreadOnly) {
    ...
    console.error("[ForensicLog API] Error:", error);
    return NextResponse.json({ error: "Failed to fetch forensic log" }, { status: 500 });
  }
}
 ...
 * Called when the Super Admin opens the notification panel.
 */
